...
public:
    ...
    uint64 CreateISM(UWorld* world, int32 meshId, const FVector& position, const FRotator& rotation, const FVector& scale);
    ...
    bool SetISMCustomData(uint64 handle, int32 customIndex, float value, bool markRenderStateDirty);
    bool SetISMNumCustomDataFloats(int32 meshId, int32 numFloats);
    bool SetISMMaterial(int32 meshId, int32 materialIndex, UMaterialInterface* material);
...
private:
    ...
    UInstancedStaticMeshComponent* GetOrCreateIsm(UWorld* world, int32 meshId);
